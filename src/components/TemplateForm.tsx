...
const templateFormSchema = z.object({
  ...
  body: z.string().min(10, 'Message body must be at least 10 characters').max(10000, 'Message body is too long'),
  ...
  {
    message: 'Scheduled date/time is required when send timing is set to scheduled',
    path: ['scheduled_for'],
  }
);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        ...
        errorMessage = getApiErrorMessage(errorText, 'Failed to create template');
      } catch (parseError) {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      try {
        const errorText = await response.text();
        console.error('API Error Response:', errorText);
        errorMessage = getApiErrorMessage(errorText, 'Failed to update template');
      } catch (parseError) {
        console.error('Error parsing error response:', parseError);
      }
      throw new Error(errorMessage);
...
export function TemplateForm({ initialData, onSuccess, onCancel }: TemplateFormProps) {
  ...
  const selectedCampaign = campaigns && Array.isArray(campaigns) 
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <Field>
        ...
        />
        <FieldDescription>
          Use Markdown syntax for formatting (e.g., **bold**, *italic*, [links](url)). Minimum 10 characters required.
        </FieldDescription>
      ...
      <SendTimingSelector
        ...
        onScheduledDateTimeChange={(value) => setValue('scheduled_for', value, { shouldValidate: true })}
      ...
      <div className="flex gap-3 justify-end pt-4 border-t">
        ...
        <Button type="submit" disabled={!isValid || isSubmitting || createMutation.isPending || updateMutation.isPending}>
